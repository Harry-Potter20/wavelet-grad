...
# =============================================================
# SESSION 16 — UNCERTAINTY-AWARE THRESHOLD CORRECTION
#
# Extends LearnedThreshold (Session 15) with uncertainty estimates.
# Network outputs (μ, log_var) instead of just μ.
#
# Key addition: confidence = exp(-uncertainty)
#   High uncertainty → stay near MAD baseline
#   Low uncertainty  → apply full learned correction
#
# Expected behaviour:
#   - More conservative than LearnedThreshold on edge cases
#   - More robust across seeds (lower variance)
#   - Slightly lower peak performance, better worst case
# =============================================================
...
def run_uncertain_threshold_adam(sigma, net, X_mean, X_std,
                                  n_steps=N_STEPS, lr=LR, seed=0,
                                  beta1=BETA1, beta2=BETA2, eps=EPS):
    """Adam with uncertainty-aware threshold correction denoising."""
    ...
    for t in range(1, n_steps + 1):
        ...
        for k in params:
            if buffer_full:
                ...
                clean_grads[k] = denoise_uncertain_batch(
...
def benchmark():
    ...
    from experiments.session15_learned_denoiser import (
        run_learned_threshold_adam, benchmark as b15
    )
    from core.learned_threshold_correction import train_correction_model

    print("Training correction model (Session 15)...")
    net, X_mean, X_std = train_correction_model(
        n_samples=10000, n_epochs=50
    )

    print("\nTraining uncertainty-aware model (Session 16)...")
    ...
    for sigma in sigma_levels:
        configs = [
            ...
            ("LearnedThreshold",
             lambda s, sg=sigma:
             run_learned_threshold_adam(
                 sg, net, X_mean, X_std, seed=s)),

            ("UncertainThreshold",
             lambda s, sg=sigma:
             run_uncertain_threshold_adam(
                 sg, unet, uX_mean, uX_std, seed=s)),
        ]
